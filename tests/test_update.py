...
def test_derives_slug_and_module():
...
def test_components_flag_updates_files(tmp_path):
...
def test_components_flag_updates_test_file_too(tmp_path):
    _make_project(tmp_path)
    project_root = tmp_path / "my-test-app"
...
def test_gcp_auth_copies_all_directory_files(tmp_path):
    _make_project(tmp_path)
    project_root = tmp_path / "my-test-app"
...
def test_files_flag_updates_specific_file(tmp_path):
    _make_project(tmp_path)
    project_root = tmp_path / "my-test-app"
    cache_file = project_root / "src" / "my_test_app" / "utils" / "cache.py"
...
def test_multiple_components_updates_all(tmp_path):
    _make_project(tmp_path)
    project_root = tmp_path / "my-test-app"
    cache_file = project_root / "src" / "my_test_app" / "utils" / "cache.py"
    logging_file = project_root / "src" / "my_test_app" / "config" / "logging_config.py"
...
def test_unknown_component_exits_with_error(tmp_path):
    _make_project(tmp_path)
    project_root = tmp_path / "my-test-app"
...
def test_missing_project_path_exits_with_error(tmp_path):
    result = runner.invoke(app, ["update", str(tmp_path / "does-not-exist")])
    assert result.exit_code != 0


def test_missing_yaml_exits_with_error(tmp_path):
