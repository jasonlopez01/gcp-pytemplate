...
def test_creates_project_with_defaults(tmp_path):
    result = _create(tmp_path)
    assert "my-service" in result
    assert (tmp_path / "my-service").is_dir()


def test_creates_api_only_project(tmp_path):
...
def test_creates_cli_only_project(tmp_path):
...
def test_writes_inputs_yaml(tmp_path):
    _create(tmp_path)
...
def test_invalid_gcp_project_returns_error_string(tmp_path):
    result = _create(tmp_path, gcp_project="INVALID PROJECT ID")
    assert result.startswith("Error:")
    assert not (tmp_path / "my-service").exists()


def test_invalid_gcp_region_returns_error_string(tmp_path):
    result = _create(tmp_path, gcp_region="not-a-region")
    assert result.startswith("Error:")


def test_defaults_output_dir_to_cwd(tmp_path, monkeypatch):
...
def test_return_string_contains_file_count(tmp_path):
    result = _create(tmp_path)
    assert "files at" in result


def test_return_string_contains_settings_summary(tmp_path):
    result = _create(tmp_path)
...
def test_updates_multiple_components(tmp_path):
    _create(tmp_path)
...
def test_invalid_project_path_returns_error_string(tmp_path):
    result = update_project(project_path=str(tmp_path / "does-not-exist"))
    assert result.startswith("Error:")


def test_missing_inputs_yaml_returns_error_string(tmp_path):
...
def test_unknown_component_returns_error_string(tmp_path):
    _create(tmp_path)
    result = update_project(
        project_path=str(tmp_path / "my-service"),
...
def test_no_components_or_files_returns_error_string(tmp_path):
    _create(tmp_path)
...
def test_returns_package_name_and_version():
    result = get_version()
...
def test_list_components_returns_known_components():
...
def test_list_components_includes_file_paths():
    result = list_components()
