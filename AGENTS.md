...
**A `.jinja` suffix on a template file is stripped when rendered.** `pyproject.toml.jinja` renders to `pyproject.toml`. Use it for any template whose real filename would make external tooling try to parse it. The suffix is stripped before `render.py` evaluates its exclusion rules, so those rules are written against the final path.
